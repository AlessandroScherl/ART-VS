...
class UnifiedEvaluator:
    ...
    def generate_summary_table(self) -> str:
        ...
        if has_tiling_data:
            headers = ['Model', 'Init Pos (cm)', 'Init Ori (°)', 'Conv.', 'Final Pos (cm)', 'Final Ori (°)', 
                       'Lowest Pos (cm)', 'Lowest Ori (°)', 'APE Pos (cm)', 'APE Ori (°)', 
                       'Pre-FPS', 'Post-FPS', 'Iter.', 'L.Ratio']
        else:
            headers = ['Model', 'Init Pos (cm)', 'Init Ori (°)', 'Conv.', 'Final Pos (cm)', 'Final Ori (°)', 
                       'Lowest Pos (cm)', 'Lowest Ori (°)', 'APE Pos (cm)', 'APE Ori (°)', 'FPS', 'Iter.', 'L.Ratio']
        ...
        for model_id in sorted(self.model_results.keys()):
            ...
            if has_tiling_data:
                ...
                row = [
                    f"Model {model_id}",
                    format_stat(initial_stats, 'mean_initial_distance', 'std_initial_distance', ".1f"),
                    format_stat(initial_stats, 'mean_initial_angle', 'std_initial_angle', ".1f"),
                    ...
                    format_stat(error_stats, 'mean_lowest_position', 'std_lowest_position', ".2f"),
                    format_stat(error_stats, 'mean_lowest_orientation', 'std_lowest_orientation', ".2f"),
                    format_stat(ape_stats, 'mean_position_ape', 'std_position_ape', ".2f"),
                    format_stat(ape_stats, 'mean_orientation_ape', 'std_orientation_ape', ".2f"),
                    ...
                    format_stat(length_stats, 'mean_length_ratio', 'std_length_ratio', ".2f")
                ]
            else:
    ...
    if args.compare and len(all_files) > 1:
        ...
        for npz_file in sorted(all_files):
            ...
            try:
                ...
                # Determine if we have tiling or general FPS
                if overall_stats['Pre FPS'] or overall_stats['Post FPS']:
                    # Tiling mode
                    if overall_stats['Pre FPS']:
                        pre_fps_str = f"{np.mean(overall_stats['Pre FPS']):.1f}"
                    if overall_stats['Post FPS']:
                        post_fps_str = f"{np.mean(overall_stats['Post FPS']):.1f}"
                else:
                    # General FPS (no tiling) - show in Pre FPS column
                    if overall_stats['General FPS']:
                        pre_fps_str = f"{np.mean(overall_stats['General FPS']):.1f}"
                
                comparison_row = [
                    overall_stats['File'][:40],  # Truncate long filenames
                    f"{overall_stats['Models']}",
        ...
        if comparison_data:
            headers = ['File', 'Models', 'Samples', 'Conv. Rate', 'Pos Err (cm)', 'Ori Err (°)', 'Pre-FPS', 'Post-FPS']
