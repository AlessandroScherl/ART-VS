...
class ViTExtractor(BaseFeatureExtractor):
    ...
    def __init__(self, model_type: str = 'dino_vits8', stride: int = 4, model: nn.Module = None, device: str = 'cuda'):
        ...
        :param model_type: A string specifying the type of model to extract from.
                          ...
                          dinov2_vitl14 | dinov2_vitg14 | vit_small_patch8_224 | vit_small_patch16_224 | 
        ...
        # Set descriptor dimension based on model type
        self._descriptor_dim = self._get_descriptor_dim_from_model_type()

    def _get_descriptor_dim_from_model_type(self) -> int:
        """Get descriptor dimension based on model type."""
        if 'vits' in self.model_type:
            ...
            # Default to small model dimension
            return 384
    ...
    def get_patch_size(self) -> int:
        ...
        return 14  # DINOv2 default patch size

    @staticmethod
    def create_model(model_type: str) -> nn.Module:
        """
        :param model_type: a string specifying which model to load. [dino_vits8 | dino_vits16 | dino_vitb8 |
                           dino_vitb16 | dinov2_vits14 | dinov2_vitb14 | dinov2_vitl14 | dinov2_vitg14 |
                           vit_small_patch8_224 | vit_small_patch16_224 | vit_base_patch8_224 | vit_base_patch16_224]
        :return: the model
        """
        torch.hub._validate_not_a_forked_repo = lambda a, b, c: True
        
        # Auto-patch DINOv2 for Python 3.8 compatibility on first load
        if 'v2' in model_type:
            try:
                model = torch.hub.load('facebookresearch/dinov2', model_type)
            except TypeError as e:
                if "unsupported operand type(s) for |" in str(e):
                    print("🔧 Detected DINOv2 Python 3.8 compatibility issue. Auto-patching...")
                    import subprocess
                    import sys
                    result = subprocess.run([sys.executable, 'patch_dinov2.py'], 
                                          capture_output=True, text=True)
                    if result.returncode == 0:
                        print("✅ DINOv2 patched successfully. Retrying model load...")
                        model = torch.hub.load('facebookresearch/dinov2', model_type)
                    else:
                        print(f"❌ Auto-patching failed: {result.stderr}")
                        raise
                else:
                    raise
            ...
            # Direct mapping of timm model names to DINOv2/DINO models via torch.hub
            model_type_dict = {
                'vit_small_patch16_224': 'dinov2_vits14',
                'vit_small_patch8_224': 'dinov2_vits14', 
                'vit_base_patch16_224': 'dinov2_vitb14',
                'vit_base_patch8_224': 'dinov2_vitb14'
            }
            if model_type in model_type_dict:
                model = torch.hub.load('facebookresearch/dinov2', model_type_dict[model_type])
            else:
                raise ValueError(f"Model type {model_type} not supported without timm. Use DINOv2 models directly.")
        return model
    ...
    def preprocess_pil(self, pil_image: Image.Image) -> torch.Tensor:
        """
        Preprocesses a PIL image before extraction.
        :param pil_image: PIL image to be preprocessed.
